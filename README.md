<h2> Hi, I'm Darshan Pawar! <img src="https://media.giphy.com/media/mGcNjsfWAjY5AEZNw6/giphy.gif" width="50"></h2>
<img align='right' src="https://media.giphy.com/media/ieyl9zmCjO4b4t6qoY/giphy.gif" width="230">
<p><em>Student at <a href="https://iiitsonepat.ac.in/">Indian Institute of Information Technology (IIIT) Sonepat</a><img src="https://media.giphy.com/media/fYSnHlufseco8Fh93Z/giphy.gif" width="30"></br>
</em></p>

[![Linkedin: Darshan JP](www.linkedin.com/in/darshan-jagdish-pawar-9b9701298)
[![GitHub Darshan Pawar](https://github.com/DarshanPawar17)


### <img src="https://media.giphy.com/media/VgCDAzcKvsR6OM0uWg/giphy.gif" width="50"> A little more about me...  

```javascript
const Darshan = {
  pronouns: "he" | "him",
  code: [Javascript ,Typescript, Python, C++],
  tools: [React , Node , Express , MongoDB , Next.js ],
  architecture: ["microservices", "MVC", "REST API"],
  Virtual Internships: {
                        JP Morgan Chase & Co.: "Software Engineer Virtual Intern",
                        Deloitte: "Data Analyst Virtual Experience"
                      },
 challenge: "I am doing the Striver A2Z 480 Module Sheet "
}
```

<img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="60"> <em><b>I love connecting with different people</b> so if you want to say <b>hi, I'll be happy to meet you more!</b> :)</em>

---
